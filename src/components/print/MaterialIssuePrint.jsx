...
export default function MaterialIssuePrint({ data, onClose }) {
  return (
    <PrintWrapper title="Material Issue Slip" onClose={onClose}>
      <div className="print-page">
        {/* Header */}
        <div style={{display:'flex',justifyContent:'space-between',borderBottom:'3px solid #1F618D',paddingBottom:12,marginBottom:16}}>
          <div>
            <div style={{fontFamily:'Syne,sans-serif',fontSize:20,fontWeight:900,color:'#1F618D'}}>{CO.name}</div>
            <div style={{fontSize:9.5,color:'#555',marginTop:3}}>{CO.addr} · GSTIN: {CO.gstin}</div>
          </div>
          <div style={{textAlign:'right'}}>
            <div style={{background:'#1F618D',color:'#fff',padding:'8px 20px',borderRadius:6,fontFamily:'Syne,sans-serif',fontSize:14,fontWeight:800,letterSpacing:.5}}>
              Material Issue Slip
            </div>
        ...
        <div style={{background:'#F8F9FA',border:'2px dashed #DDD',borderRadius:8,padding:32,textAlign:'center',color:'#999',marginBottom:20}}>
          ...
          <div style={{fontSize:13,fontWeight:600,color:'#714B67'}}>Material Issue Slip</div>
