...
export default function PPConfigurator() {
  ...
  return (
    <div>
      ...
      {/* Step Progress */}
      <div style={{display:'flex',gap:'0',marginBottom:'24px',background:'#fff',borderRadius:'8px',border:'1px solid var(--odoo-border)',overflow:'hidden'}}>
        {STEPS.map((s,i)=>(
          <div key={s} style={{flex:1,padding:'12px 16px',display:'flex',alignItems:'center',gap:'10px',
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'14px'}}>
            {BUSINESS_TYPES.map(b=>(
              <div key={b.key} onClick={()=>setBizType(b.key)}
                style={{background:'#fff',borderRadius:'12px',padding:'20px',cursor:'pointer',transition:'all .2s',
                ...
                <div style={{fontSize:'12px',color:'var(--odoo-gray)',lineHeight:'1.6'}}>{b.desc}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:'10px',marginBottom:'20px'}}>
            {INDUSTRY_SUBTYPES.map(s=>(
              <div key={s.key} onClick={()=>{setSubType(s.key);setSelProcesses([]);setChargeBy('')}}
                style={{background:'#fff',borderRadius:'8px',padding:'12px 10px',textAlign:'center',cursor:'pointer',transition:'all .18s',
                    ...
                    {sub.chargeBy.map(c=>(
                      <div key={c} onClick={()=>setChargeBy(c)}
                        style={{padding:'6px 16px',borderRadius:'7px',border:'1.5px solid',cursor:'pointer',fontSize:'12px',fontWeight:'500',
                          ...
                          background:chargeBy===c?'var(--odoo-purple)':'#fff',
                          color:chargeBy===c?'#fff':'var(--odoo-text)'}}>
                        {c}
                      </div>
                    ))}
                  </div>
                ...
                <div style={{display:'flex',flexWrap:'wrap',gap:'0',alignItems:'center'}}>
                  {selProcesses.map((p,i)=>(
                    <React.Fragment key={p}>
                      <div style={{padding:'6px 12px',background:'var(--odoo-purple)',color:'#fff',borderRadius:'4px',fontSize:'12px',fontWeight:'600'}}>{i+1}. {p}</div>
                      {i<selProcesses.length-1&&<div style={{fontSize:'16px',color:'var(--odoo-gray)',padding:'0 4px'}}>→</div>}
                    </React.Fragment>
                  ))}
                </div>
            ...
            <div className="fi-panel">
              ...
              <div className="fi-panel-body">
                {[
                  ...
                  <div key={k} style={{display:'flex',justifyContent:'space-between',padding:'8px 0',borderBottom:'1px solid var(--odoo-border)',fontSize:'12px'}}>
                    <span style={{color:'var(--odoo-gray)',fontWeight:'600'}}>{k}</span>
              ...
              <div className="fi-panel-body">
                {selProcesses.map((p,i)=>(
                  <div key={p} style={{display:'flex',alignItems:'center',gap:'10px',padding:'6px 0',borderBottom:'1px solid var(--odoo-border)'}}>
                    <div style={{width:'22px',height:'22px',borderRadius:'50%',background:'var(--odoo-purple)',color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:'700',flexShrink:0}}>{i+1}</div>
                    <span style={{fontSize:'12px',fontWeight:'600'}}>{p}</span>
                    {i===0&&<span style={{marginLeft:'auto',fontSize:'10px',background:'#D1ECF1',color:'#0C5460',padding:'2px 7px',borderRadius:'8px',fontWeight:'700'}}>START</span>}
                    {i===selProcesses.length-1&&<span style={{marginLeft:'auto',fontSize:'10px',background:'#D4EDDA',color:'#155724',padding:'2px 7px',borderRadius:'8px',fontWeight:'700'}}>END</span>}
                  </div>
                ))}
              </div>
            </div>
