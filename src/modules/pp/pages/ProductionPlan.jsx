...
export default function ProductionPlan() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [rP, rN, rI] = await Promise.all([
        ...
        fetch(`${BASE_URL}/items`,        { headers: hdr2() }),
      ])
      const [dP, dN, dI] = await Promise.all([rP.json(), rN.json(), rI.json()])
      // FIX 1: No SEED fallback — show real data or empty state
      setPlans(dP.data || [])
  ...
  // FIX 2: Separate FG and all items for dropdown
  // itemType field: FG = Finished Goods, SFG = Semi-Finished, RM = Raw Material
  const fgItems = items.filter(i => i.itemType === 'FG' || i.itemType === 'SFG')
    ...
    <div>
        ...
        <div style={{border:'2px solid #714B67',borderRadius:8,overflow:'hidden',marginBottom:16}}>
          ...
          <div style={{padding:16,background:'#FDF8FC'}}>
            <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr',gap:12,marginBottom:12}}>
              <div>
                <label style={lbl}>Item / Product * (FG / Semi-FG only)</label>
                {/* FIX 3: Use fgItems filtered by itemType */}
                ...
                {/* FIX 4: If no FG items, show manual input with warning */}
                {!fgItems.length && (
                  <div>
                    <input style={{...inp,marginTop:6}} value={form.itemName} onChange={fSet('itemName')} placeholder="Type item name manually..."/>
                    <span style={{fontSize:10,color:'#856404',fontWeight:700}}>
                      ⚠ No FG items found. Go to Item Master → create item with type = Finished Goods
                    </span>
                  </div>
                )}
              </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr',gap:12,marginBottom:12}}>
              ...
              <div>
                ...
                <input style={inp} value={form.workCenter} onChange={fSet('workCenter')} placeholder="e.g. IMM-150T"/>
              </div>
              <div>
      ...
      {loading ? (
        ...
        <div style={{padding:60,textAlign:'center',color:'#6C757D',border:'2px dashed #E0D5E0',borderRadius:8,background:'#fff'}}>
          ...
          <div style={{fontSize:12,marginTop:6}}>Click "+ New Plan" to create your first production plan.</div>
        ...
        Object.entries(grouped).map(([week, weekItems]) => (
          <div key={week} style={{marginBottom:20}}>
            <div style={{display:'flex',alignItems:'center',gap:10,padding:'6px 0',borderBottom:'2px solid #EDE0EA',marginBottom:10}}>
              ...
              <div style={{fontSize:11,color:'#6C757D'}}>{weekItems.length} plans</div>
              <div style={{fontSize:11,color:'#6C757D',marginLeft:'auto'}}>
                Total: {weekItems.reduce((a,p)=>a+parseFloat(p.plannedQty||0),0).toFixed(0)} units planned
              </div>
            </div>
            ...
            {weekItems.map(p => {
              ...
              return (
                <div key={p.id} style={{
                  ...
                  marginBottom:8, overflow:'hidden', boxShadow:'0 1px 4px rgba(0,0,0,.05)',
                }}>
