...
export default function QuotList() {
  ...
  return (
    <div>
      ...
      <div className="sd-fb">
        <div className="sd-fs">
          <input placeholder="Search QT #, customer..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      ...
      {viewMode === 'normal' && (
        <div className="dc">
          <table className="sd-tbl">
            <thead>
              <tr>
                <th>QT #</th><th>Date</th><th>Valid Till</th>
                ...
                <tr key={q.id} style={{ cursor:'pointer' }} onClick={() => navigate('/sd/quotations/' + q.id)}>
                  ...
                  <td onClick={e => e.stopPropagation()} style={{ display:'flex', gap:4 }}>
                    ...
                    {q.status !== 'cancelled' && q.status !== 'won' && (
                      <button
                        style={{ padding:'3px 10px', fontSize:11, fontWeight:700, borderRadius:4, border:'1px solid #714B67', background:'#EDE0EA', color:'#714B67', cursor:'pointer' }}
                        onClick={() => navigate('/sd/quotations/' + q.id + '?edit=true')}
                      >Edit</button>
                    )}
                    {q.status === 'pending' && (
            ...
            return (
              <div key={q.id} onClick={() => navigate('/sd/quotations/' + q.id)}
                ...
                <div style={{ padding:'10px 16px', background:'var(--odoo-bg)',
                  borderBottom:'1px solid var(--odoo-border)', display:'flex',
                ...
                <div onClick={e => e.stopPropagation()} style={{ padding:'8px 16px', background:'var(--odoo-bg)',
                  borderTop:'1px solid var(--odoo-border)', display:'flex', gap:8 }}>
                  <button className="act-btn-view" onClick={() => navigate('/sd/quotations/' + q.id)}>View</button>
                  {q.status === 'pending' && (
