...
export default function StaffAttendanceRegister() {
  ...
  const load = () => {
    ...
    fetch(`${BASE}/edu/attendance/staff/register?${params}`, { headers:hdr2() })
  ...
  return (
    <div style={{fontFamily:'DM Sans,sans-serif'}}>
      <div style={{background:'#fff',borderBottom:'1px solid #E8E0E8',padding:'10px 16px',marginBottom:12,
        ...
        <div style={{fontSize:18,fontWeight:800,color:'#1E8449'}}>📋 Staff Attendance Register</div>
        <button onClick={()=>nav('/edu/attendance/staff')}
          style={{padding:'7px 16px',background:'#1E8449',color:'#fff',border:'none',borderRadius:5,
      ...
      <div style={{background:'#fff',border:'1px solid #E8E0E8',borderRadius:8,overflow:'hidden'}}>
        {loading ? (
          ...
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead><tr>
              <th style={th}>Date</th><th style={th}>Total Staff</th><th style={th}>Present</th>
              <th style={th}>Absent</th><th style={th}>Late</th><th style={th}>Half Day</th>
              <th style={th}>Attendance %</th><th style={th}></th>
            </tr></thead>
            <tbody>
              {sessions.map((s,i) => (
                <tr key={i} onClick={()=>nav(`/edu/attendance/staff?date=${s.date}`)}
                  style={{cursor:'pointer',background:i%2===0?'#fff':'#F8FDF8'}}
                  onMouseEnter={e=>e.currentTarget.style.background='#E8F5E9'}
                  onMouseLeave={e=>e.currentTarget.style.background=i%2===0?'#fff':'#F8FDF8'}>
                  ...
                  <td style={{...td,color:'#1E8449',fontWeight:700}}>View / Edit →</td>
                </tr>
              ))}
              {sessions.length===0 && <tr><td colSpan={8} style={{...td,textAlign:'center',color:'#aaa',padding:40}}>No attendance marked in this date range</td></tr>}
            </tbody>
          </table>
        )}
      </div>
