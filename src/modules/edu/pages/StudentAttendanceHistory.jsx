...
export default function StudentAttendanceHistory() {
  ...
  const searchStudents = async (q) => {
    ...
    const r = await fetch(`${BASE}/edu/students?search=${encodeURIComponent(q)}&institutionId=${instId}`, { headers:hdr2() })
    const d = await r.json()
    setResults((d.data||[]).slice(0,8))
  }

  const pickStudent = (s) => {
    setSelStudent(s); setResults([]); setSearch(s.name)
  }

  useEffect(() => {
    if (!selStudent) return
    setLoading(true)
    const params = new URLSearchParams({ studentId:selStudent.id, dateFrom, dateTo })
    fetch(`${BASE}/edu/attendance/student?${params}`, { headers:hdr2() })
      .then(r=>r.json()).then(d => setRecords(d.data||[]))
      .finally(() => setLoading(false))
  }, [selStudent, dateFrom, dateTo])
  ...
  return (
    <div style={{fontFamily:'DM Sans,sans-serif'}}>
      <div style={{background:'#fff',borderBottom:'1px solid #E8E0E8',padding:'10px 16px',marginBottom:12}}>
        <div style={{fontSize:18,fontWeight:800,color:'#6E2C00'}}>🔍 Student Attendance History</div>
        <div style={{fontSize:11,color:'#888',marginTop:2}}>Look up one student's full attendance record over any date range</div>
      </div>
      ...
      <div style={{background:'#fff',border:'1px solid #E8E0E8',borderRadius:8,padding:16,marginBottom:12}}>
        <div style={{position:'relative',maxWidth:400,marginBottom:12}}>
          <input placeholder='Search by name or admission no...' value={search}
            onChange={e=>searchStudents(e.target.value)} style={{...inp,width:'100%',boxSizing:'border-box'}} />
          {results.length>0 && (
      ...
      {!selStudent ? (
        <div style={{padding:60,textAlign:'center',background:'#fff',borderRadius:8,border:'1px solid #E8E0E8',color:'#aaa'}}>
          Search and select a student to view their attendance history
        </div>
      ) : loading ? (
        ...
        <>
          ...
          <div style={{background:'#fff',border:'1px solid #E8E0E8',borderRadius:8,overflow:'hidden'}}>
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead><tr><th style={th}>Date</th><th style={th}>Status</th><th style={th}>Remarks</th></tr></thead>
              <tbody>
                {records.map(r => {
                ...
                {records.length===0 && <tr><td colSpan={3} style={{...td,textAlign:'center',color:'#aaa',padding:30}}>No attendance records in this date range</td></tr>}
              </tbody>
            </table>
          </div>
        </>
