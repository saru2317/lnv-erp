...
const PAYMENT_TERMS = ['Immediate','7 Days','15 Days','30 Days','45 Days','60 Days','90 Days','Advance']

const SAMPLE = [
  { id:1, code:'MFR',  name:'Manufacturer',         category:'Manufacturer',      paymentTerms:'30 Days', active:true  },
  { id:2, code:'TRD',  name:'Trader / Dealer',       category:'Trader',            paymentTerms:'15 Days', active:true  },
  { id:3, code:'SVC',  name:'Service Provider',      category:'Service Provider',  paymentTerms:'30 Days', active:true  },
  { id:4, code:'IMP',  name:'Importer',              category:'Importer',          paymentTerms:'Advance', active:true  },
  { id:5, code:'LAB',  name:'Labour Contractor',     category:'Labour Contractor', paymentTerms:'7 Days',  active:true  },
  { id:6, code:'GVT',  name:'Government Supplier',   category:'Government',        paymentTerms:'60 Days', active:true  },
  { id:7, code:'MSME', name:'MSME Vendor',           category:'Manufacturer',      paymentTerms:'45 Days', active:true  },
  { id:8, code:'OTH',  name:'Other',                 category:'Other',             paymentTerms:'30 Days', active:false },
]
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18 }}>
        <div>
          ...
          <p style={{ fontSize:12, color:'#6C757D', margin:'3px 0 0' }}>
            MDM › Supplier Type &nbsp;|&nbsp; {rows.length} types configured
          </p>
        </div>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse' }}>
          <thead>
            <tr style={{ background:'#F8F4F8', borderBottom:'2px solid #E0D5E0' }}>
              {['Code','Supplier Type','Category','Default Payment Terms','Status','Actions'].map(h=>(
            ...
            {filtered.map((r,i)=>(
              <tr key={r.id} style={{ borderBottom:'1px solid #F0EEF0', background:i%2===0?'#fff':'#FDFBFD' }}
                ...
                <td style={{ padding:'10px 14px' }}>
                  <span style={{ padding:'3px 9px', borderRadius:10, fontSize:11, fontWeight:600, background:'#EDE0EA', color:'#714B67' }}>
                    {r.category}
                  </span>
                </td>
                <td style={{ padding:'10px 14px', fontSize:12, color:'#6C757D' }}>{r.paymentTerms}</td>
                <td style={{ padding:'10px 14px' }}>
          ...
          <div style={{ background:'#fff', borderRadius:10, width:460,
            ...
            <div style={{ background:'#714B67', padding:'14px 20px',
              ...
              <h3 style={{ color:'#fff', fontFamily:'Syne,sans-serif', fontSize:15, fontWeight:700, margin:0 }}>
                {editId?'Edit Supplier Type':'New Supplier Type'}
              </h3>
            ...
            <div style={{ padding:20, display:'flex', flexDirection:'column', gap:14 }}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                <div>
                  ...
                  <input {...inp('code')} placeholder="e.g. MFR" maxLength={10}/>
                </div>
                <div>
              ...
              <div>
                <label style={{ fontSize:11, fontWeight:700, color:'#1C1C1C', display:'block', marginBottom:5, textTransform:'uppercase', letterSpacing:.4 }}>Supplier Type Name *</label>
                <input {...inp('name')} placeholder="e.g. Manufacturer"/>
              </div>
              <div>
                <label style={{ fontSize:11, fontWeight:700, color:'#1C1C1C', display:'block', marginBottom:5, textTransform:'uppercase', letterSpacing:.4 }}>Default Payment Terms</label>
                <select {...inp('paymentTerms')} style={{...inp('paymentTerms').style, cursor:'pointer'}}>
