...
export default function CostCenters() {
  ...
  const ROWS = [
    ['CC-001', 'Production Department', 'Direct', 'PP', '50,00,000', 'Active'],
    ['CC-002', 'Quality Control', 'Direct', 'QM', '8,00,000', 'Active'],
    ['CC-003', 'Maintenance', 'Direct', 'PM', '12,00,000', 'Active'],
    ['CC-004', 'Sales and Marketing', 'Indirect', 'SD', '15,00,000', 'Active'],
    ['CC-005', 'Purchase Department', 'Indirect', 'MM', '5,00,000', 'Active'],
    ['CC-006', 'Finance and Accounts', 'Overhead', 'FI', '8,00,000', 'Active'],
    ['CC-007', 'Human Resources', 'Overhead', 'HCM', '6,00,000', 'Active'],
    ['CC-008', 'Transport', 'Support', 'TM', '10,00,000', 'Active'],
  ]
  ...
  const save = () => {
    if (!form.code) return toast.error('CC Code required')
  ...
  return (
    <div>
      <div className="fi-lv-hdr">
        <div className="fi-lv-title">Cost Centers <small>SAP: KS01</small></div>
      ...
      <div style={{ padding:'8px 12px', background:'#E6F7F7', border:'1px solid #00A09D',
        ...
        <strong>Cost Centers</strong> — Department-wise cost allocation and profit centers
      </div>
      ...
      {showNew && (
        <div style={{ background:'#fff', border:'1px solid var(--odoo-purple)',
          ...
          <div style={{ padding:'10px 16px', background:'var(--odoo-purple)',
            ...
            <span style={{ color:'#fff', fontWeight:700, fontSize:13 }}>New Cost Centers</span>
          ...
          <div style={{ padding:16, display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'10px 16px' }}>
            <div className="sd-fg">
              <label style={{ fontSize:11, fontWeight:600, color:'var(--odoo-gray)',
                display:'block', marginBottom:4 }}>CC Code</label>
            ...
            <div className="sd-fg">
              ...
              <input style={{ ...inp }}
                value={form.ccType || ''} onChange={e => setForm(p => ({ ...p, ccType: e.target.value }))} />
            </div>
            <div className="sd-fg">
              <label style={{ fontSize:11, fontWeight:600, color:'var(--odoo-gray)',
                display:'block', marginBottom:4 }}>Module</label>
              <input style={{ ...inp }}
                value={form.module || ''} onChange={e => setForm(p => ({ ...p, module: e.target.value }))} />
            </div>
            <div className="sd-fg">
              <label style={{ fontSize:11, fontWeight:600, color:'var(--odoo-gray)',
                display:'block', marginBottom:4 }}>Budget</label>
              <input style={{ ...inp }}
        ...
        <tbody>
          {filtered.map((r, i) => (
            <tr key={i} style={{ background: i%2===0 ? '#fff' : '#FAFAFA' }}>
              ...
              <td>
                <button
                  onClick={() => toast.success('Edit CostCenters')}
