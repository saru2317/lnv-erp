...
export default function QualityMaster() {
  ...
  const ROWS = [
    ['IP-001', 'Incoming RM Inspection', 'Inspection Plan', 'IS 9844', '—', 'Active'],
    ['IP-002', 'In-Process Coating Inspection', 'Inspection Plan', 'Customer Spec', '—', 'Active'],
    ['IP-003', 'Final Inspection Plan', 'Inspection Plan', 'IS 9844', '—', 'Active'],
    ['DC-001', 'Dimensional Non-Conformance', 'Defect Code', 'Drawing Spec', 'Critical', 'Active'],
    ['DC-002', 'Surface Defect Colour Variation', 'Defect Code', 'RAL Standard', 'Major', 'Active'],
    ['DC-003', 'Coating Thickness Out of Range', 'Defect Code', 'ISO 2178', 'Major', 'Active'],
    ['DC-004', 'Adhesion Failure', 'Defect Code', 'ASTM D3359', 'Critical', 'Active'],
  ]
  ...
  return (
    <div>
      <div className="fi-lv-hdr">
        <div className="fi-lv-title">Quality Masters <small>SAP: QP01</small></div>
      ...
      <div style={{ padding:'8px 12px', background:'#E6F7F7', border:'1px solid #00A09D',
        ...
        <strong>Quality Masters</strong> — Inspection plans, defect codes and sampling procedures
      </div>
      ...
      {showNew && (
        <div style={{ background:'#fff', border:'1px solid var(--odoo-purple)',
          ...
          <div style={{ padding:'10px 16px', background:'var(--odoo-purple)',
            ...
            <span style={{ color:'#fff', fontWeight:700, fontSize:13 }}>New Quality Masters</span>
          ...
          <div style={{ padding:16, display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'10px 16px' }}>
            ...
            <div className="sd-fg">
              <label style={{ fontSize:11, fontWeight:600, color:'var(--odoo-gray)',
                display:'block', marginBottom:4 }}>Standard</label>
              <input style={{ ...inp }}
                value={form.standard || ''} onChange={e => setForm(p => ({ ...p, standard: e.target.value }))} />
            </div>
            <div className="sd-fg">
              <label style={{ fontSize:11, fontWeight:600, color:'var(--odoo-gray)',
                display:'block', marginBottom:4 }}>Notes</label>
              <input style={{ ...inp }}
        ...
        <tbody>
          {filtered.map((r, i) => (
            <tr key={i} style={{ background: i%2===0 ? '#fff' : '#FAFAFA' }}>
              ...
              <td>
                <button
                  onClick={() => toast.success('Edit QualityMaster')}
