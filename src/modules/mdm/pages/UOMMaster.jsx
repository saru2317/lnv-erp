...
export default function UOMMaster() {
  ...
  const ROWS = [
    ['KG', 'Kilogram', 'Weight', 'KGM', '3', 'Active'],
    ['NOS', 'Numbers', 'Count', 'H87', '0', 'Active'],
    ['LTR', 'Litre', 'Volume', 'LTR', '3', 'Active'],
    ['MTR', 'Metre', 'Length', 'MTR', '3', 'Active'],
    ['SET', 'Set', 'Group', 'SET', '0', 'Active'],
    ['ROLL', 'Roll', 'Count', 'RO', '0', 'Active'],
    ['BOX', 'Box', 'Packaging', 'BX', '0', 'Active'],
    ['LOT', 'Lot', 'Batch', 'LT', '0', 'Active'],
    ['TON', 'Metric Tonne', 'Weight', 'TNE', '3', 'Active'],
    ['CBM', 'Cubic Metre', 'Volume', 'MTQ', '3', 'Active'],
  ]
  ...
  const save = () => {
    if (!form.code) return toast.error('UOM Code required')
  ...
  return (
    <div>
      <div className="fi-lv-hdr">
        <div className="fi-lv-title">Unit of Measure <small>SAP: CUNI</small></div>
      ...
      <div style={{ padding:'8px 12px', background:'#E6F7F7', border:'1px solid #00A09D',
        ...
        <strong>Unit of Measure</strong> — All units used across purchasing, sales and inventory
      </div>
      ...
      {showNew && (
        <div style={{ background:'#fff', border:'1px solid var(--odoo-purple)',
          ...
          <div style={{ padding:'10px 16px', background:'var(--odoo-purple)',
            ...
            <span style={{ color:'#fff', fontWeight:700, fontSize:13 }}>New Unit of Measure</span>
          ...
          <div style={{ padding:16, display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'10px 16px' }}>
            <div className="sd-fg">
              <label style={{ fontSize:11, fontWeight:600, color:'var(--odoo-gray)',
                display:'block', marginBottom:4 }}>UOM Code</label>
            ...
            <div className="sd-fg">
              <label style={{ fontSize:11, fontWeight:600, color:'var(--odoo-gray)',
                display:'block', marginBottom:4 }}>ISO Code</label>
              <input style={{ ...inp, fontFamily:'DM Mono,monospace' }}
                value={form.isoCode || ''} onChange={e => setForm(p => ({ ...p, isoCode: e.target.value }))} />
            </div>
            <div className="sd-fg">
              <label style={{ fontSize:11, fontWeight:600, color:'var(--odoo-gray)',
                display:'block', marginBottom:4 }}>Decimal Places</label>
              <input style={{ ...inp }}
                value={form.decimal || ''} onChange={e => setForm(p => ({ ...p, decimal: e.target.value }))} />
            </div>
        ...
        <tbody>
          {filtered.map((r, i) => (
            <tr key={i} style={{ background: i%2===0 ? '#fff' : '#FAFAFA' }}>
              ...
              <td>
                <button
                  onClick={() => toast.success('Edit UOMMaster')}
