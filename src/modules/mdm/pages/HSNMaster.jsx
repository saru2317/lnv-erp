...
export default function HSNMaster() {
  ...
  const ROWS = [
    ['9999 99 00', 'Surface Treatment Services', 'SAC', '18%', '9%', '9%', 'Active'],
    ['9988 10 00', 'Job Work Services', 'SAC', '18%', '9%', '9%', 'Active'],
    ['8448 59 90', 'Textile Machinery Parts', 'HSN', '18%', '9%', '9%', 'Active'],
    ['8448 49 00', 'Needles for Machines', 'HSN', '18%', '9%', '9%', 'Active'],
    ['3208 10 00', 'Paints and Varnishes', 'HSN', '18%', '9%', '9%', 'Active'],
    ['3919 10 00', 'Self-Adhesive Tape', 'HSN', '12%', '6%', '6%', 'Active'],
    ['7315 11 00', 'Roller Chain', 'HSN', '18%', '9%', '9%', 'Active'],
  ]
  ...
  const save = () => {
    if (!form.code) return toast.error('HSN/SAC Code required')
  ...
  return (
    <div>
      <div className="fi-lv-hdr">
        <div className="fi-lv-title">HSN / SAC Code Master <small>SAP: VK11</small></div>
      ...
      <div style={{ padding:'8px 12px', background:'#E6F7F7', border:'1px solid #00A09D',
        ...
        <strong>HSN / SAC Code Master</strong> — GST tax classification codes for goods and services
      </div>
      ...
      {showNew && (
        <div style={{ background:'#fff', border:'1px solid var(--odoo-purple)',
          ...
          <div style={{ padding:'10px 16px', background:'var(--odoo-purple)',
            ...
            <span style={{ color:'#fff', fontWeight:700, fontSize:13 }}>New HSN</span>
          ...
          <div style={{ padding:16, display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'10px 16px' }}>
            <div className="sd-fg">
              <label style={{ fontSize:11, fontWeight:600, color:'var(--odoo-gray)',
                display:'block', marginBottom:4 }}>HSN/SAC Code</label>
            ...
            <div className="sd-fg">
              <label style={{ fontSize:11, fontWeight:600, color:'var(--odoo-gray)',
                display:'block', marginBottom:4 }}>IGST %</label>
              <input style={{ ...inp }}
                value={form.igst || ''} onChange={e => setForm(p => ({ ...p, igst: e.target.value }))} />
            </div>
            <div className="sd-fg">
              <label style={{ fontSize:11, fontWeight:600, color:'var(--odoo-gray)',
                display:'block', marginBottom:4 }}>CGST %</label>
              <input style={{ ...inp }}
                value={form.cgst || ''} onChange={e => setForm(p => ({ ...p, cgst: e.target.value }))} />
            </div>
            <div className="sd-fg">
              <label style={{ fontSize:11, fontWeight:600, color:'var(--odoo-gray)',
                display:'block', marginBottom:4 }}>SGST %</label>
              <input style={{ ...inp }}
                value={form.sgst || ''} onChange={e => setForm(p => ({ ...p, sgst: e.target.value }))} />
            </div>
        ...
        <tbody>
          {filtered.map((r, i) => (
            <tr key={i} style={{ background: i%2===0 ? '#fff' : '#FAFAFA' }}>
              ...
              <td>
                <button
                  onClick={() => toast.success('Edit HSNMaster')}
