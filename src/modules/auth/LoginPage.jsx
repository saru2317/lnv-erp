...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const demoUser = DEMO_USERS.find(u => u.email === email && u.password === pass)
      if (demoUser) {
        setMode('demo')
        demoLogin('admin')
        navigate('/home')
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex',
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column',
        alignItems:'center', padding:'60px 60px 24px',
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column',
          ...
          <div style={{ marginTop:32, display:'flex', gap:10, flexWrap:'wrap', justifyContent:'center', maxWidth:480 }}>
            {MOD_PILLS.map(p => (
              <span key={p} style={{ background:'rgba(255,255,255,.1)',
                border:'1px solid rgba(255,255,255,.15)', borderRadius:20,
                padding:'5px 14px', color:'rgba(255,255,255,.7)',
      ...
      <div style={{ width:480, background:'#fff', display:'flex', flexDirection:'column',
        justifyContent:'center', padding:'44px 40px',
        position:'relative', zIndex:1, boxShadow:'-16px 0 48px rgba(0,0,0,.22)' }}>

        {/* Color bar top */}
        <div style={{ position:'absolute', top:0, left:0, right:0, height:3,
          background:'linear-gradient(90deg,#714B67,#E06F39,#00A09D)' }} />

        {/* Header */}
        <div style={{ marginBottom:22 }}>
          <div style={{ display:'flex', alignItems:'center', gap:9, marginBottom:16 }}>
            <div style={{ width:34, height:34, background:'linear-gradient(135deg,#E06F39,#F5C518)',
              borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center',
              fontFamily:'Syne,sans-serif', fontSize:13, fontWeight:800, color:'#fff' }}>LNV</div>
            <span style={{ fontFamily:'Syne,sans-serif', fontSize:18, fontWeight:800, color:'#212529' }}>
              LNV ERP
            </span>
          </div>
          <h2 style={{ fontFamily:'Syne,sans-serif', fontSize:22, fontWeight:700,
            color:'#212529', marginBottom:4 }}>Welcome Back</h2>
          <p style={{ color:'#6C757D', fontSize:12 }}>
            {tab==='signin'
              ? 'Sign in to your LNV ERP account to continue'
        ...
        <div style={{ display:'flex', borderBottom:'2px solid #DDD5D0', marginBottom:24 }}>
          {[['signin','Sign In'],['register','Register Company']].map(([k,l]) => (
            <div key={k} onClick={() => { setTab(k); setError('') }}
              style={{ flex:1, padding:10, textAlign:'center', fontSize:13,
                fontWeight:600, cursor:'pointer', transition:'color .2s',
        ...
        {tab==='signin' && <>
            ...
            <div style={{ position:'relative' }}>
              ...
              <span onClick={()=>setShowPass(s=>!s)}
                style={{ position:'absolute', right:12, top:'50%',
                  ...
                  fontSize:15, color:'#6C757D', userSelect:'none' }}>
                {showPass ? '🙈' : '👁️'}
              </span>
            </div>
          </div>
          ...
          <div style={{ display:'flex', justifyContent:'space-between',
            ...
            <label style={{ display:'flex', alignItems:'center', gap:6,
              fontSize:12, color:'#6C757D', cursor:'pointer' }}>
              <input type="checkbox" checked={remember}
                onChange={e=>setRemember(e.target.checked)}
                style={{ accentColor:'#714B67' }} />
              Remember me
            </label>
            <a href="#" style={{ fontSize:12, color:'#714B67',
              fontWeight:600, textDecoration:'none' }}>Forgot password?</a>
          </div>

          {error && (
            <div style={{ background:'#FDEDEC', border:'1px solid #F5B7B1',
        ...
        {tab==='register' && <>
          ...
          <div style={{ marginBottom:12 }}>
            ...
            <input style={inp()} type="email" value={rEmail}
          ...
          <div style={{ marginBottom:20 }}>
            <label style={lbl}>Password</label>
            <input style={inp()} type="password" value={rPass}
          ...
          <button
            ...
            style={{ width:'100%', padding:12, background:'#714B67',
              color:'#fff', border:'none', borderRadius:6,
              fontFamily:'DM Sans,sans-serif', fontSize:14, fontWeight:700,
