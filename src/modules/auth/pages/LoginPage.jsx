...
// ─────────────────────────────────────────────────────
const DEMO_USERS = [
  { email:'admin@lnverp.com',      password:'admin123',  role:'admin',      name:'Saravana Kumar', label:'Super Admin' },
  { email:'manager@lnverp.com',    password:'lnv@2025',  role:'manager',    name:'Ramesh P',       label:'Plant Manager' },
  { email:'accounts@lnverp.com',   password:'lnv@2025',  role:'accounts',   name:'Priya S',        label:'Accounts' },
  { email:'operations@lnverp.com', password:'lnv@2025',  role:'operations', name:'Karthik M',      label:'Operations' },
  { email:'hr@lnverp.com',         password:'lnv@2025',  role:'hr',         name:'Kavitha R',      label:'HR Manager' },
  { email:'sales@lnverp.com',      password:'lnv@2025',  role:'sales',      name:'Vijay T',        label:'Sales Rep' },
]
...
function BgShape({ size, top, left, right, bottom, delay=0 }) {
...
export default function LoginPage() {
  ...
  const handleLogin = () => {
    ...
    if (!user) { setError('Invalid credentials. Click a demo card below to auto-fill.'); return }
    setLoading(true)
    setTimeout(() => {
      demoLogin(user.role)
      navigate('/home')
  ...
  const inp = (extra={}) => ({
    width:'100%', padding:'10px 14px', border:'1.5px solid #DDD5D0',
  ...
  const focusIn  = e => { e.target.style.borderColor='#714B67'; e.target.style.boxShadow='0 0 0 3px rgba(113,75,103,.12)' }
  const focusOut = e => { e.target.style.borderColor='#DDD5D0'; e.target.style.boxShadow='none' }
  ...
  return (
    <div style={{
      ...
      {/* Animated BG shapes */}
      <BgShape size={400} top="-100px"  left="-100px" delay={0}   />
      <BgShape size={300} bottom="-80px" right="460px" delay={1.5} />
      <BgShape size={200} top="45%"     right="440px" delay={3}   />

      {/* ══════════ LEFT PANEL ══════════ */}
      <div style={{ flex:1, display:'flex', flexDirection:'column',
        alignItems:'center', padding:'60px 60px 24px', position:'relative', zIndex:1, minHeight:'100vh' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', width:'100%' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:0 }}>
          <div style={{
            ...
            boxShadow:'0 16px 48px rgba(224,111,57,.5)',
          }}>LNV</div>
          ...
          <p style={{ color:'rgba(255,255,255,.55)', fontSize:13, marginTop:8,
            letterSpacing:3, textTransform:'uppercase', margin:'8px 0 0' }}>
            by LNV Infotech Soft Solutions
          </p>
        </div>
          ...
          {FEATURES.map(f => (
            <div key={f.title}
              style={{ background:'rgba(255,255,255,.08)', border:'1px solid rgba(255,255,255,.12)',
                borderRadius:12, padding:16, backdropFilter:'blur(8px)', cursor:'default',
      ...
      {/* Copyright — always at bottom in flow */}
      <div style={{ fontSize:11, color:'rgba(255,255,255,.4)', textAlign:'center', paddingTop:24 }}>
      ...
      {/* ══════════ RIGHT FORM PANEL ══════════ */}
      <div style={{
        width:460, background:'#fff', position:'relative', zIndex:1,
        boxShadow:'-16px 0 48px rgba(0,0,0,.25)',
        overflowY:'auto', display:'flex', flexDirection:'column',
      }}>
      <div style={{ flex:1, display:'flex', flexDirection:'column', justifyContent:'center', padding:'32px 44px' }}>
        {/* LNV mini logo + name */}
        <div style={{ marginBottom:18 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
            <div style={{
              ...
              background:'linear-gradient(135deg,#E06F39,#F5C518)',
              display:'flex', alignItems:'center', justifyContent:'center',
              fontFamily:'Syne,sans-serif', fontSize:15, fontWeight:800, color:'#fff',
            }}>LNV</div>
            <span style={{ fontFamily:'Syne,sans-serif', fontSize:20, fontWeight:800, color:'#1C1C1C' }}>LNV ERP</span>
          </div>

          <h2 style={{ fontFamily:'Syne,sans-serif', fontSize:26, fontWeight:700, color:'#1C1C1C', margin:'0 0 5px', minHeight:36 }}>
            {tab==='signin' ? 'Welcome Back ' : 'Register Company '}
          </h2>
          <p style={{ color:'#6C757D', fontSize:13, margin:0, minHeight:20 }}>
            {tab==='signin' ? 'Sign in to your LNV ERP account to continue' : 'Start your 30-day free trial — no credit card required'}
          </p>
        </div>
        ...
        <div style={{ display:'flex', borderBottom:'2px solid #DDD5D0', marginBottom:24 }}>
          {[['signin','Sign In'],['register','Register Company']].map(([k,l]) => (
            <div key={k} onClick={() => { setTab(k); setError('') }}
              style={{
                flex:1, padding:10, textAlign:'center', fontSize:13, fontWeight:600,
        ...
        {tab === 'signin' && <>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
            <label style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'#6C757D', cursor:'pointer' }}>
              <input type="checkbox" checked={remember} onChange={e=>setRemember(e.target.checked)} style={{ accentColor:'#714B67' }} />
              Remember me
            </label>
          ...
          {error && (
            <div style={{ background:'#FDEDEC', border:'1px solid #F5B7B1', borderRadius:6,
              padding:'10px 14px', fontSize:12, color:'#C0392B', marginBottom:12 }}>⚠️ {error}
            </div>
          )}
          ...
          <button onClick={handleLogin} disabled={loading}
            style={{
              ...
              background: loading ? '#9E7D96' : '#714B67',
          ...
          <div style={{ marginTop:10, background:'#FDF8FC', border:'1px solid #E8D5E5', borderRadius:6, padding:'12px 14px' }}>
            <div style={{ fontSize:11, fontWeight:700, color:'#714B67', marginBottom:8,
              textTransform:'uppercase', letterSpacing:.5 }}>🔑 Demo Credentials — click to fill</div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:4, marginBottom:6 }}>
              {DEMO_USERS.map(u => (
                <div key={u.email}
                  onClick={() => { setEmail(u.email); setPass(u.password); setError('') }}
                  style={{ padding:'6px 9px', borderRadius:5, cursor:'pointer', fontSize:11,
        ...
        {tab === 'register' && <>
          <div style={{ marginBottom:12 }}>
            ...
            <input style={inp()} value={rComp} onChange={e=>setRComp(e.target.value)}
              placeholder="e.g. Gayathri Marketing Pvt Ltd" onFocus={focusIn} onBlur={focusOut} />
          </div>
          ...
          <div style={{ marginBottom:12 }}>
            ...
            <input style={inp()} type="email" value={rEmail} onChange={e=>setREmail(e.target.value)}
              placeholder="admin@yourcompany.com" onFocus={focusIn} onBlur={focusOut} />
          </div>

          <div style={{ marginBottom:20 }}>
            <label style={lbl}>Password</label>
            <input style={inp()} type="password" value={rPass} onChange={e=>setRPass(e.target.value)}
              placeholder="Min 8 characters" onFocus={focusIn} onBlur={focusOut} />
          </div>

          <button
            onClick={() => { alert('Registration noted! Use demo credentials to explore.'); setTab('signin') }}
